...
def cifar10_dataloaders(BATCH_SIZE=32):
    ...
    train_transform = Compose([RandomCrop(32, padding=4), RandomHorizontalFlip(), ToTensor(), Normalize(mean=mean, std=std),])
    val_test_transform = Compose([ToTensor(), Normalize(mean=mean, std=std),])
    # Datasets
    ...
    val_dataloader = DataLoader(dataset=val_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=num_workers, drop_last=True, pin_memory=True,)
