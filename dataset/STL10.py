...
from preprocess.mean_std_dataset import mean_std_dataset
...
def stl10_datasets_dataloaders(data="datasets", BATCH_SIZE=32, RANDOM_SEED=42):
    ...
    """
    train_dataset = STL10(root=os.getcwd(), split='train', download=True, transform=ToTensor())
    train_mean, train_std = mean_std_dataset(train_dataset)
    train_transform = Compose([ToTensor(), Normalize(mean=train_mean, std=train_std),])
    train_dataset.transform = train_transform
    test_dataset_full = STL10(root=os.getcwd(), split='test', download=False, transform=ToTensor())
    val_test_mean, val_test_std = mean_std_dataset(test_dataset_full)
    test_dataset_full.transform = Compose([ToTensor(), Normalize(mean=val_test_mean, std=val_test_std)])
